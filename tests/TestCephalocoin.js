...
contract('Cephalocoin constructor', (accounts) => {
  it('Creation should emit Transfer event', async () => {
    const coin = await Cephalocoin.new( 1, { from: accounts[0] });
...
contract('Cephalocoin', () => {
  const [wallet, walletTo, otherWallet] = new ethereumWaffle.MockProvider().getWallets();
  let token;

  beforeEach(async () => {
    token = await ethereumWaffle.deployContract(wallet, Cephalocoin, [1]);
  });
